...
export default function DashboardLayout({
  ...
  return (
    <div className="min-h-screen flex bg-gray-100">
      <aside className={`${sidebarOpen ? "w-64" : "w-20"} bg-gradient-to-b from-[#2D6A4F] to-[#1a4a35] text-white transition-all duration-300 flex flex-col flex-shrink-0`}>
        ...
        <nav className="flex-1 p-4 space-y-2">
          {navItems.map(item => (
            <button
              ...
              className={`flex items-center gap-3 px-3 py-3 rounded-xl cursor-pointer transition w-full text-left ${pathname === item.path ? "bg-white/15 text-white" : "text-white/70 hover:bg-white/10"}`}
            >
